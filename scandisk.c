...
#define CLUSTER_IS_USED (1)
#define CLUSTER_IS_POINTED (1 << 1)
...
void check_free_cluster(int[] *cluster_info, uint8_t *image_buf, struct bpb33 *bpb) {
    uint16_t cluster = 0;
    for (int i = 2; i < bpb -> bpbSectors + 2; i++) {
        cluster = get_12masked_fat_entry(i, image_buf, bpb);
        if (cluster == CLUST_FREE) {
            *(cluster_info[i]) = *(cluster_info[i]) & 0x
        }
    }
